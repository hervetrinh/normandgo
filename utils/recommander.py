...
class RecommendationSystem:
    def __init__(self, embedding_model_name, data, config_path, device=None):
        """
        Initialize the recommendation system with model path, data, and config.
    ...
    def extract_features(self):
        """
        Extract features from the model using the combined features.
        """
    ...
    def get_recommendations_with_geo(self, user_lat=None, user_lon=None, tsid=None):
        ...
        Args:
            ...
            tsid (str, optional): TSID of the reference item. Defaults to None.

        Returns:
            dict: Dictionary containing the name of the input monument and DataFrame with recommended items and distances in meters.
        """
        ...
        for idx in monument_indices:
            ...
            photos = str(self.data.iloc[idx]['PHOTO']).split(' ## ') if 'PHOTO' in self.data.columns and pd.notna(self.data.iloc[idx]['PHOTO']) and self.data.iloc[idx]['PHOTO'] else []
