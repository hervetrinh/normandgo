...
openagenda = OpenAgendaEvents(config['openagenda']['api_key'], config['openagenda']['agenda_uids'])
rag_agent = RAGAgent(
...
def tables():
    ...
    return render_template('tables.html', table_data=paginated_data, next_url=next_url, prev_url=prev_url)
...
def feedback():
    ...
    classifier = VerbatimClassifier(llm=LLMUtils().get_llm())
...
def charts():
    ...
    bar_labels = [label.capitalize() for label in sentiment_counts.index.tolist()]  # Mettre en majuscule
    ...
    recent_negatives = (
        ...
        .head(5)[['date', 'nom_monument', 'feedback']]  
    )
...
def restaurants():
    ...
    return render_template('restaurants.html', restaurants=restos, page=page, total_pages=total_pages)
